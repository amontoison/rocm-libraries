...
### Added
* Added a unified preconditioner backsolve family that hides the triangular-solve details (fill mode, diagonal type, and transpose selection) behind a single high-level call per factorization type, gated by the `BUILD_WITH_PRECONDITIONERS_SOLVE` build option (`ROCSPARSE_WITH_PRECONDITIONERS_SOLVE`). The first routine, `rocsparse_spic0_solve`, solves `A X = B` from an incomplete Cholesky factor (`A ~ L L^T`) by chaining the forward (`L Y = B`) and backward (`L^T X = Y`) sweeps, and supports multiple right-hand sides. Its descriptor is configured through `rocsparse_spic0_solve_descr_create`/`_destroy`, `rocsparse_spic0_solve_set_input`, and `rocsparse_spic0_solve_get_output`, with a two-stage (`rocsparse_spic0_solve_stage_analysis`, `rocsparse_spic0_solve_stage_solve`) workflow and `rocsparse_spic0_solve_buffer_size`. The descriptor also exposes iterative-refinement configuration (`rocsparse_spic0_solve_input_refinement_steps`, `rocsparse_spic0_solve_input_matrix`) for a future refinement loop.
* Added `rocsparse_spilu0_solve`, the unified backsolve for an incomplete LU factorization (`A ~ L U`, combined factor), chaining the forward (`L Y = B`, unit lower) and backward (`U X = Y`, non-unit upper) sweeps in a single call with multiple right-hand-side support.
* Added `rocsparse_spildlt0_solve`, the unified backsolve for an incomplete LDL' factorization (`A ~ L D L^H`, Hermitian, combined factor), chaining the forward sweep (`L Y = B`, unit lower), the diagonal scale (`Y <- D^{-1} Y`) and the backward sweep (`L^H X = Y`, unit lower conjugate transpose) in a single call with multiple right-hand-side support.
